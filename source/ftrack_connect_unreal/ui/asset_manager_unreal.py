...
class Ui_AssetManager(object):
    ...
    def setupUi(self, AssetManager):
        ...
        self.AssertManagerTreeView = QtWidgets.QTreeView(AssetManager)
        self.AssertManagerTreeView.setFrameShape(QtWidgets.QFrame.Box)
        self.AssertManagerTreeView.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.AssertManagerTreeView.setSelectionBehavior(
            QtWidgets.QAbstractItemView.SelectRows
        )
        self.AssertManagerTreeView.setMaximumWidth(300)
        self.horizontalLayout2.addWidget(self.AssertManagerTreeView)
        ...
        self.horizontalLayout2.addWidget(self.AssertManagerTableWidget)
...
class FtrackAssetManagerDialog(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, connector=None):
        ...
        # set central widget for mainWindow so I can add menuBar for debug options
        self.setCentralWidget(self.centralwidget)
        self.centralwidget.setLayout(self.verticalMainLayout)
        ...
        self.setWindowTitle('ftrackAssetManager - beta 0.02')
...
class AssetManagerWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent, task=None, connector=None):
        ...
        self.ui.AssertManagerTreeView.setSortingEnabled(True)
        self.ui.AssertManagerTreeView.setAlternatingRowColors(True)
        #self.ui.AssertManagerTreeView.setColumnCount(1)
        #self.ui.AssertManagerTreeView.header().setVisible(False)
        self.ui.AssertManagerTreeView.setColumnWidth(1,500)


        self.ui.AssertManagerTableWidget.setSortingEnabled(True)
        self.ui.AssertManagerTableWidget.setShowGrid(False)
        self.ui.AssertManagerTableWidget.setAlternatingRowColors(True)
    ...
    def refreshAssetManager(self):
        ...
        self.ui.AssertManagerTreeView.setSortingEnabled(False)

        proxy = ftrack_connect.ui.model.entity_tree.EntityTreeProxyModel(self)
        self.model = ftrack_connect.ui.model.entity_tree.EntityTreeModel(
        ...
        self.ui.AssertManagerTreeView.setModel(proxy)
        self.ui.AssertManagerTreeView.setSortingEnabled(True)

        self.ui.AssertManagerTreeView.setColumnWidth(0, 180)

        self.assets = self.connector.getAssets()

        self.component_ids = []

        for component_id, _ in self.assets:
            if component_id:
                self.component_ids.append(component_id)

        self.ui.AssertManagerTreeView.clicked.connect(self.on_click)
    ...
    def on_click(self,item):
        ...
        index = self.ui.AssertManagerTreeView.selectedIndexes()[0]

        type_index = self.ui.AssertManagerTreeView.selectedIndexes()[1]
        #item = self.model.itemFromIndex(index)

        project = index

        while project.parent().isValid():
            project = project.parent()

        assets = []
        component_map = dict()

        if 'Episode' in type_index.data():

            # shots = self.session.query('Shot where parent.name is "{0}" and project.name is "{1}"'.format(index.data(), project.data()))
            # for shot in shots:
            #     components = self.session.query('Component where version.asset.context_id is "{0}"'.format(shot['id']))
            #
            #     for component in components:
            #         for component_id, name_in_scene in self.assets:
            #             if component_id == component['id']:
            #                 assets.append((component_id,name_in_scene))
            #
            #                 component_map[component_id] = component

            components = self.session.query('Component where version.asset.parent.parent.name is "{0}" and version.asset.parent.project.name is "{1}" and version.asset.type.name is "Animation"'.format(index.data(), project.data()))
        ...
        if 'Shot' in type_index.data():

            shot = self.session.query('Shot where name is "{0}" and project.name is "{1}" and parent.name is "{2}"'.format(index.data(), project.data(),index.parent().data())).first()
        ...
        if 'Asset Build' in type_index.data():
            print index.data()
            asset_build = self.session.query('AssetBuild where name is "{0}" and project.name is "{1}" and parent.name is "{2}"'.format(index.data(), project.data(),index.parent().data())).first()

            print asset_build
            components = self.session.query('Component where version.asset.context_id is "{0}"'.format(asset_build['id']))
            print components[0]
            for component in components:
                print component['id']
                for component_id, name_in_scene in self.assets:
